...
const protect = async (req, res, next) => {
  ...
  if (
    ...
    try {
      ...
      // 🔍 ফিক্স: টোকেনের ভেতর আইডি 'id' নাকি '_id' নামে আছে তা নিশ্চিত করা
      const userId = decoded.id || decoded._id;

      // ৩. ডাটাবেজ থেকে ইউজারকে খুঁজে বের করা (পাসওয়ার্ড ছাড়া)
      req.user = await User.findById(userId).select('-password');

      // যদি টোকেন ভ্যালিড হয় কিন্তু কোনো কারণে ডাটাবেজে ইউজার না থাকে
      if (!req.user) {
  ...
  // ৪. যদি কোনো টোকেন পাঠানোই না হয় (এখানে return যুক্ত করা হয়েছে)
  if (!token) {
