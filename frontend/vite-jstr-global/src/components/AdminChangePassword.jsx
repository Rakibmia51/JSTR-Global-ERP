...
const AdminChangePassword = () => {
  ...
  // আইডি ভেরিফাই করে ব্যাকএন্ড থেকে নাম আনার ফাংশন
  const fetchAccountName = async (idValue) => {
    const id = idValue || targetId;
    if (!id || id.trim().length < 4) return;
    ...
    try {
      ...
      // ডাইনামিক ডিস্ট্রিক্ট আইডি (e.g. CHP-0001) ও মার্কেটিং আইডি দুটোই এই রুট হ্যান্ডেল করবে
      const response = await axios.get(
        `${SERVER_URL}/api/users/admin/get-name/${id.trim()}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  // পাসওয়ার্ড রিসেট ফর্ম সাবমিট হ্যান্ডলার
  const handleSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      // ব্যাকএন্ড পাসওয়ার্ড রিসেট পুট API কল
      const response = await axios.put(
        `${SERVER_URL}/api/users/admin/reset-password`,
        { targetId, newPassword }, 
        config
      ...
      if (response.data.success) {
        toast.success(response.data.message || 'Password updated successfully!');
        // সফলভাবে পাসওয়ার্ড পরিবর্তন হলে স্টেটগুলো রিসেট হবে
        setTargetId('');
        setAccountName('');
        setAccountType('');
        setNewPassword('');
    ...
    <div style={styles.wrapper}>
      ...
      <div style={styles.container}>
        <div style={styles.header}>
          ...
          <h2 style={styles.title}>Admin Password Reset</h2>
          <p style={styles.subtitle}>Verify account identity and override access credentials securely.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.inputGroup}>
            ...
            <div style={styles.inputWrapper}>
              <input
                ...
                onChange={(e) => {
                  ...
                  // আইডি টাইপ করার সাথে সাথে আগের নাম ও টাইপ মুছে যাবে যাতে ভুল ডাটা সাবমিট না হয়
                  if (accountName) {
                    setAccountName('');
                    setAccountType('');
                  }
                ...
                onBlur={() => fetchAccountName()} 
                required
                placeholder="e.g. MKT-0002 or CHP-0001"
              ...
              <button 
                ...
                onClick={() => fetchAccountName()} 
                style={styles.searchButton}
                disabled={nameLoading || !targetId.trim()}
              >
                {nameLoading ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Search size={16} />}
              </button>
            </div>
            ...
            {accountName && (
              <div style={styles.namePreviewBox}>
                <UserCheck size={16} color="#166534" />
                <span style={styles.nameText}>
                  <strong>{accountType}:</strong> {accountName}
                </span>
              </div>
          ...
          <button 
            ...
            style={{ 
              ...styles.submitBtn, 
              backgroundColor: (loading || !accountName) ? '#93c5fd' : '#2563eb', 
              cursor: (loading || !accountName) ? 'not-allowed' : 'pointer'
            }}
          >
            {loading ? 'Processing...' : 'Reset Password'}
...
// নতুন মডার্ন ব্লু এবং স্লেট থিম সিএসএস
const styles = {
  wrapper: { 
    display: 'flex', 
    justifyContent: 'center', 
  ...
  container: { 
    ...
    backgroundColor: '#ffffff', 
    borderRadius: '16px', 
    boxShadow: '0 10px 15px -3px rgb(37 99 235 / 0.05), 0 4px 6px -4px rgb(37 99 235 / 0.05)', 
    padding: '32px 24px', 
    boxSizing: 'border-box', 
