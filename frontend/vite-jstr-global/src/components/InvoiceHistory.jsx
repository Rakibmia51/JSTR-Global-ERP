...
const InvoiceList = () => {
  ...
  // 🔄 সার্ভার-সাইড এপিআই কল লজিক
  const fetchInvoices = useCallback(async () => {
    setLoading(true);
    try {
      // রিকোয়েস্ট পাঠানোর সময় সরাসরি প্যারামিটার অবজেক্ট পাস করা
      const response = await axios.get(`${SERVER_URL}/api/invoices`, {
        params: {
          page: currentPage,
          limit: limit,
          search: searchTerm.trim(), // স্পেস ট্রিম করা হলো
          status: statusFilter
        }
      });
  ...
  }, [currentPage, limit, searchTerm, statusFilter, SERVER_URL]);

  // ডিপেন্ডেন্সি অ্যারে ট্র্যাকিং ফিক্স
  useEffect(() => {
    fetchInvoices();
  }, [currentPage, searchTerm, statusFilter, fetchInvoices]);

  // ইনপুট বক্স লাইভ চেঞ্জ হ্যান্ডলার
  const handleSearchChange = (e) => {
    setSearchTerm(e.target.value);
    setCurrentPage(1); // টাইপ করার সাথে সাথে পেজ ১ এ রিসেট হবে
  };

  // ড্রপডাউন চেঞ্জ হ্যান্ডলার
  const handleStatusChange = (e) => {
    setStatusFilter(e.target.value);
    setCurrentPage(1); // ফিল্টার চেঞ্জ হলে পেজ ১ এ রিসেট হবে
  };
  ...
  if (loading && invoices.length === 0) {
    return <div className="text-center py-12 text-gray-500 font-medium bg-gray-50 min-h-screen">Loading Invoices...</div>;
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-xs border border-gray-100">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Invoice Management ({totalCount})</h2>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">View, search, and manage all sales invoices.</p>
        </div>
        <button 
          onClick={() => navigate('/admin-panel/accounting/create-invoice')} 
          className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-md transition transform active:scale-95 flex items-center justify-center gap-2 text-sm"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
          Create New Invoice
      ...
      {/* সার্চ এবং ফিল্টার বার */}
      <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-200/60">
        <div className="relative sm:col-span-2">
          <input 
            type="text"
            placeholder="Search by Invoice No, Dealer, or Customer..."
            value={searchTerm}
            onChange={handleSearchChange} // ইনস্ট্যান্ট লাইভ সার্চ এবং পেজ রিসেট হ্যান্ডলার
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-black"
          />
          <svg className="w-4 h-4 text-gray-400 absolute left-3.5 top-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        </div>
        <div>
          <select
            value={statusFilter}
            onChange={handleStatusChange} // লাইভ স্ট্যাটাস পরিবর্তন হ্যান্ডলার
            className="w-full bg-white border border-gray-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-gray-700"
          >
            <option value="All">All Status</option>
            <option value="Paid">Paid</option>
        ...
        {invoices.map((inv) => (
          <div key={inv._id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-2xs space-y-3">
            ...
            <div className="text-xs text-gray-600 space-y-1">
              <p><span className="font-semibold text-gray-400">Client:</span> {inv.dealer ? inv.dealer.name : inv.customerName || 'Walk-in Customer'}</p>
              <p><span className="font-semibold text-gray-400">Date:</span> {new Date(inv.createdAt).toLocaleDateString('en-BD')}</p>
              <p><span className="font-semibold text-gray-400">Grand Total:</span> <span className="font-bold text-gray-900">৳{inv.grandTotal?.toLocaleString() || 0}</span></p>
              <p><span className="font-semibold text-gray-400">Due Amount:</span> <span className="font-bold text-rose-600">৳{inv.dueAmount?.toLocaleString() || 0}</span></p>
            </div>
          ...
          <tbody className="text-sm divide-y divide-gray-50 text-gray-700 font-medium">
            {invoices.map((inv) => (
              <tr key={inv._id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="py-4 px-5 text-gray-950 font-bold">৳{inv.grandTotal?.toLocaleString() || 0}</td>
                <td className="py-4 px-5 text-emerald-600">৳{inv.paidAmount?.toLocaleString() || 0}</td>
                <td className="py-4 px-5 text-rose-600">৳{inv.dueAmount?.toLocaleString() || 0}</td>
                <td className="py-4 px-5">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border
                    ${inv.paymentStatus === 'Paid' ? 'bg-green-50 text-green-700 border-green-200' : ''}
                    ${inv.paymentStatus === 'Partially Paid' ? 'bg-amber-50 text-amber-700 border-amber-200' : ''}
      ...
      {/* 📊 সুন্দর এবং আধুনিক পেজিনেশন প্যানেল */}
      {invoices.length > 0 && (
        <div className="p-4 bg-white border border-gray-100 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
          <span className="text-sm font-medium text-gray-500">
            Showing Page <span className="text-indigo-600 font-bold">{currentPage}</span> of <span className="font-bold text-gray-800">{totalPages}</span> ({totalCount} total invoices)
          </span>
          <div className="flex items-center space-x-2">
            <button 
              ...
              className="p-2 border border-gray-300 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed bg-white text-gray-700 hover:bg-gray-100 transition shadow-xs"
            >
              ...
              if (pageNum === 1 || pageNum === totalPages || Math.abs(currentPage - pageNum) <= 1) {
                return (
                  <button
                    ...
                    className={`px-3.5 py-1.5 text-sm font-semibold rounded-xl transition border ${
                      currentPage === pageNum 
