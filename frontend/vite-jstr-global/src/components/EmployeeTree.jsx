...
import { 
  FaChevronRight, FaChevronDown,FaBuilding ,FaSearch,FaSitemap , FaUserAlt, FaUsers, 
  FaUserTie , FaNetworkWired 
} from "react-icons/fa";
...
const TreeBranch = ({ node, searchTerm, level = 0 }) => {
  ...
  // 💥 ফিক্স ১: লেভেল অনুযায়ী আলাদা আলাদা চমৎকার কালার প্যালেট জেনারেশন (চাইল্ড কালার স্কিম)
  const getLevelTheme = (lvl) => {
    const themes = [
      { bg: "#ffffff", border: "#e2e8f0", icon: "linear-gradient(135deg, #4f46e5, #3730a3)", text: "#0f172a" }, // রুট লেভেল (Level 0) - Indigo
      { bg: "#f0fdfa", border: "#99f6e4", icon: "linear-gradient(135deg, #0d9488, #115e59)", text: "#115e59" }, // লেভেল ১ - Teal
      { bg: "#eff6ff", border: "#bfdbfe", icon: "linear-gradient(135deg, #2563eb, #1d4ed8)", text: "#1e40af" }, // লেভেল ২ - Blue
      { bg: "#faf5ff", border: "#d8b4fe", icon: "linear-gradient(135deg, #9333ea, #6b21a8)", text: "#581c87" }, // লেভেল ৩ - Purple
      { bg: "#fff7ed", border: "#ffedd5", icon: "linear-gradient(135deg, #ea580c, #c2410c)", text: "#7c2d12" }, // লেভেল ৪ - Orange
      { bg: "#f0fdf4", border: "#bbf7d0", icon: "linear-gradient(135deg, #16a34a, #15803d)", text: "#14532d" }, // লেভেল ৫ - Green
    ];
    // যদি লেভেল ৫ এর চেয়ে বেশি নিচে যায়, তবে কালার লুপিং হবে
    return themes[lvl % themes.length];
  };

  const theme = getLevelTheme(level);

  // পজিশন র‍্যাংক কালার ব্যাজ
  const getPositionBadgeStyle = (pos) => {
    const p = pos?.toUpperCase()?.trim();
    switch (p) {
      case "ED": return { bg: "#faf5ff", text: "#a855f7", border: "#d8b4fe" };
      case "NSM": return { bg: "#fff7ed", text: "#f97316", border: "#ffedd5" };
      case "SM": return { bg: "#f0fdf4", text: "#16a34a", border: "#bbf7d0" };
      case "SDSM": return { bg: "#f0fdfa", text: "#0d9488", border: "#99f6e4" };
      case "DSM": return { bg: "#ecfdf5", text: "#059669", border: "#a7f3d0" };
      case "RSM": return { bg: "#eff6ff", text: "#2563eb", border: "#bfdbfe" };
      case "AM": return { bg: "#f0fdf4", text: "#15803d", border: "#bbf7d0" };
  ...
  return (
    <div style={{ margin: "10px 0", position: "relative", boxSizing: "border-box", width: "100%" }}>
      ...
      <div
        ...
        style={{
          ...
          justifyContent: "space-between",
          padding: "12px 14px",
          // সার্চে ম্যাচ করলে হাইলাইট, নাহলে লেভেল অনুযায়ী আলাদা আলাদা ব্যাকগ্রাউন্ড
          background: matchesSearch && searchTerm ? "#f0fdf4" : theme.bg,
          border: matchesSearch && searchTerm ? "2px solid #10b981" : `1px solid ${theme.border}`,
          borderRadius: "14px",
          cursor: hasChildren ? "pointer" : "default",
        ...
        className="tree-card-hover"
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px", minWidth: 0, flex1: 1 }}>
          {/* ড্রপডাউন অ্যারো */}
          <span style={{ color: "#94a3b8", display: "flex", flexShrink: 0 }}>
            {hasChildren ? (isExpanded ? <FaChevronDown size={12} /> : <FaChevronRight size={12} />) : <span style={{ width: "4px", height: "4px", background: "#cbd5e1", borderRadius: "50%", marginLeft: "4px" }} />}
          </span>

          {/* অবতার আইকন (লেভেল অনুযায়ী আলাদা কালার গ্রাডিয়েন্ট) */}
          <div style={{
            width: "34px",
            height: "34px",
            borderRadius: "50%",
            ...
            {hasChildren ? <FaUserTie size={13} /> : <FaUserAlt size={12} />}
          </div>

          {/* নাম, আইডি, পজিশন এবং টোটাল সেলস ডিসপ্লে */}
          <div style={{ display: "flex", flexDirection: "column", minWidth: 0, flex1: 1 }}>
            <span style={{ fontSize: "14px", fontWeight: "600", color: theme.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {node.name}
            </span>
            
            <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap", marginTop: "4px" }}>
              {/* কাস্টম আইডি */}
              <span style={{ fontSize: "10px", color: "#64748b", fontWeight: "600" }}>{node.idNo}</span>
        ...
        {/* ডাইরেক্ট টিম কাউন্টার ব্যাজ */}
        {hasChildren && (
          <div style={{ display: "flex", alignItems: "center", gap: "4px", background: "#f1f5f9", padding: "4px 8px", borderRadius: "8px", flexShrink: 0, marginLeft: "6px" }}>
      ...
      {/* 💥 ফিক্স ২: ডাউন-বাই-ডাউন লেআউট কন্টেইনার (marginLeft এবং ড্যাশড বর্ডার বাদ দেওয়া হয়েছে) */}
      {hasChildren && isExpanded && (
        <div style={{ 
          marginTop: "6px", 
          paddingLeft: "0px", // ডানে আর কোনো মার্জিন/প্যাডিং যাবে না
          borderLeft: "none",  // সাইডে চলে যাওয়া রোধ করতে বর্ডার লাইন রিমুভ করা হলো
          boxSizing: "border-box", 
          width: "100%",
          display: "flex",
          flexDirection: "column",
          gap: "2px" // চাইল্ড নোডগুলো সোজা নিচে নিচে একটার পর একটা বসবে
        }}>
          {node.children.map((child) => (
            <TreeBranch 
              key={child._id} 
              ...
              level={level + 1} // লেভেল ১ প্লাস হয়ে পরবর্তী ডাইনামিক কালার ট্রিগার করবে
            />
          ))}
        </div>
      )}
