...
const DealerList = () => {
    ...
    const [filteredDealers, setFilteredDealers] = useState([]); // Live Filtered Data
    const [searchTerm, setSearchTerm] = useState('');           // Search Input Text
    const [selectedStatus, setSelectedStatus] = useState('');   // Filter by Status Dropdown
    const [statusList, setStatusList] = useState([]);          // Unique Status List
    
    const navigate = useNavigate();

// 1. Fetch Dealers Data from Backend API
    useEffect(() => {
        const fetchDealers = async () => {
            try {
                // Backend API End-point URL (Port matched to your server)
                const response = await axios.get('http://localhost:3000/api/dealers');
                if (response.data.success) {
                    setDealers(response.data.data);
                    setFilteredDealers(response.data.data);
                    console.log('Fetched dealers:', response.data.data);
                }
            } catch (error) {
   ...
   // 2. Build unique status list once dealers data is loaded
    useEffect(() => {
        if (dealers.length > 0) {
  ...
  // 3. Live Filtering Logic (Triggers when searchTerm, selectedStatus, or dealers state changes)
    useEffect(() => {
        let tempDealers = [...dealers];

        // Apply Live Text Search (Name, Dealer ID, Mobile Phone No)
        if (searchTerm.trim() !== '') {
            const currSearch = searchTerm.toLowerCase();
            tempDealers = tempDealers.filter((dealer) =>
                dealer.name?.toLowerCase().includes(currSearch) ||
                dealer.dealerId?.toLowerCase().includes(currSearch) || // Fixed from idNo to dealerId
                dealer.mobilePhoneNo?.includes(currSearch)             // Fixed from mobileNo to mobilePhoneNo
            );
        }
