...
const EmployeeList = () => {
  const [employees, setEmployees] = useState([]);
  const [totalCount, setTotalCount] = useState(0); // মোট কতজন ইউজার আছে তার সংখ্যা
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  ...
  // ১. ড্রপডাউন মেনুর জন্য আলাদাভাবে ইউনিক ডিপার্টমেন্টগুলো নিয়ে আসা
  const fetchDepartments = async () => {
    try {
      const token = localStorage.getItem('token');
      // দ্রষ্টব্য: আপনার ব্যাকএন্ডে আলাদা ডিপার্টমেন্ট রাউট থাকলে এটি কাজ করবে
      // যদি আলাদা রাউট না থাকে, তবে এটিকে কমেন্ট আউট করে ম্যানুয়াল অ্যারেও রাখতে পারেন
  ...
  // ২. সার্ভার থেকে পেজিনেশন ও কুয়েরি প্যারামিটারসহ ফিল্টার করা ডাটা আনা
  const fetchEmployees = async () => {
    setLoading(true);
    const token = localStorage.getItem('token'); 
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/users/all?${queryParams}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      }); 
      ...
      if (result.success) {
        setEmployees(result.data); // এখন এখানে শুধুমাত্র ২০টি ডাটা থাকবে, ২০৫২টি নয়!
  ...
  // প্রথমবার পেজ লোড হলে ডিপার্টমেন্ট নিয়ে আসবে
  useEffect(() => {
    fetchDepartments();
  }, []);

  // পেজ নম্বর অথবা ডিপার্টমেন্ট ড্রপডাউন চেঞ্জ হলে অটোমেটিক ডাটা রি-লোডের ট্রিগার
  useEffect(() => {
    fetchEmployees();
  }, [currentPage, selectedDepartment]);

  // সার্চ সাবমিট হ্যান্ডলার (এন্টার চাপলে বা সার্চ বাটনে ক্লিক করলে কার্যকর হবে)
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    setCurrentPage(1); // নতুন করে সার্চ করলে যেন ১ম পেজ থেকে শুরু হয়
    fetchEmployees();
  };

  // এডিট স্টেটের ভ্যারিয়েবলসমূহ
  const [editingEmployee, setEditingEmployee] = useState(null);
  const [showEditModal, setShowEditModal] = useState(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('আপনি কি নিশ্চিত যে এই কর্মচারীর ডাটা ডিলিট করতে চান?')) {
      try {
        ...
        fetchEmployees(); // ডিলিট সফল হওয়ার পর বর্তমান পেজের ডাটা রিফ্রেশ করা
        alert('সফলভাবে ডিলিট হয়েছে!');
      } catch (err) {
        alert(err.response?.data?.message || 'ডিলিট করতে সমস্যা হয়েছে');
      }
  ...
  if (loading && employees.length === 0) return <div className="p-6 text-center text-lg font-semibold text-gray-700">Loading Employees...</div>;
  if (error) return <div className="p-6 text-center text-red-500 font-semibold">{error}</div>;
  ...
  return (
    <div className="mt-15 p-0 bg-gray-50 min-h-screen">
      <div className="max-w-8xl mx-auto bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
        ...
        {/* টেবিল হেডার / শিরোনাম */}
        <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white flex justify-between items-center">
          <h2 className="text-xl font-bold tracking-wide">All Employees List ({totalCount})</h2>
          <button 
            onClick={() => navigate('/admin-panel/employees/add')} 
            className="bg-white text-indigo-600 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-indigo-50 transition">
            + Add New Staff
        ...
        <form onSubmit={handleSearchSubmit} className="p-4 bg-gray-100 border-b border-gray-200 grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          {/* <div>
            ...
            >
              <Search className="w-4 2-4" /> Search
            </button>
          </div> */}
        </form>
        ...
        <div>
            ...
            {/* ১. মোবাইল ভিউ: ছোট স্ক্রিনে দেখাবে, বড় স্ক্রিনে (lg) হাইড থাকবে */}
          <div className="block lg:hidden divide-y divide-gray-200">
            {employees.map((emp) => (
              <div key={emp._id} className="p-4 bg-white hover:bg-gray-50 transition space-y-3">
                
                {/* প্রোফাইল পিকচার, নাম, আইডি এবং স্ট্যাটাস */}
                <div className="flex items-center space-x-3">
                  {emp.photo ? (
                    <img 
                      src={`${SERVER_URL}/${emp.photo}`} 
                      alt={emp.name} 
                      className="w-12 h-12 rounded-full object-cover border border-gray-300"
                    />
                  ) : (
                    /* 🔠 ছবি না থাকলে নামের প্রথম অক্ষর দিয়ে গোল আইকন */
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 border border-indigo-200 flex items-center justify-center text-white font-bold text-sm shadow-sm uppercase">
                      {emp.name ? emp.name.charAt(0) : 'E'}
                    </div>
                  )}
                  <div>
                ...
                {/* কন্টাক্ট, ডিপার্টমেন্ট এবং রোল ইনফো গ্রিড */}
                <div className="grid grid-cols-2 gap-2 text-xs text-gray-600 bg-gray-50 p-2.5 rounded-lg">
                  <div>
                    <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Contact</span>
                    <div className="text-gray-900 font-medium">{emp.mobileNo}</div>
                  ...
                  <div className="mt-1">
                    <span className="font-semibold text-gray-400 block uppercase tracking-wider text-[10px]">Role</span>
                    <span className="capitalize font-medium text-gray-900">{emp.role}</span>
                  </div>
                </div>

                {/* অ্যাকশন বাটনসমূহ (View, Edit, Delete) */}
          ...
          {/* ২. ডেক্সটপ ভিউ: বড় স্ক্রিনে টেবিল দেখাবে */}
          <div className="hidden lg:block overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-700 uppercase text-xs font-bold border-b border-gray-200">
        ...
        {employees.length > 0 && (
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
            <span className="text-sm font-medium text-gray-600">
              Showing Page <span className="text-indigo-600 font-bold">{currentPage}</span> of <span className="font-bold">{totalPages}</span> ({totalCount} total staff)
            </span>
        ...
        {/* যদি ডাটাবেস বা ফিল্টারে কোনো এমপ্লয়ী না পাওয়া যায় */}
